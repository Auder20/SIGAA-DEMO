#!/usr/bin/env python
"""
IMPORTADOR DE APORTES ADEMACOR - VERSIÓN OPTIMIZADA
Basado en el importador de aportes individuales pero adaptado para ADEMACOR.

Características:
- ✅ Detección automática de tipo de aporte (ADEMACOR, FAMECOR)
- ✅ Caché de afiliados y sueldos (evita consultas N+1)
- ✅ Bulk upsert para máximo rendimiento
- ✅ Procesamiento vectorizado con pandas
- ✅ Creación automática de sueldos si no existen
...
class AporteAdemacorImporter:
    ...
    def _cargar_cache_afiliados(self):
        """Carga afiliados ADEMACOR Y sueldos existentes"""
        logger.info("Cargando cachés ADEMACOR...")
        start = time.time()
    ...
    def _procesar_lote_bulk(self, df_lote: pd.DataFrame) -> Dict[str, int]:
        ...
        try:
            with transaction.atomic():
                # 1. IDENTIFICAR Y CREAR SUELDOS FALTANTES
                sueldos_crear = []
    ...
    def procesar_archivo(self) -> Dict[str, Any]:
        ...
        logger.info("="*70)
        logger.info(f"INICIANDO IMPORTACIÓN ADEMACOR - AÑO {self.anio}")
        logger.info("="*70)
        ...
        try:
            ...
            for i in range(0, len(df_limpio), self.batch_size):
                ...
                if lote_num % 5 == 0 or lote_num == total_lotes:
            ...
            logger.info("="*70)
            logger.info(f"{self.tipo_aporte} ADEMACOR COMPLETADO")
...
def importar_aportes_ademacor_completos(archivo_ademacor: str, archivo_famecor: str = None, anio: int = None, batch_size: int = None) -> Dict[str, Any]:
    """Importa ambos tipos de aportes ADEMACOR (ADEMACOR y FAMECOR)"""
    inicio = time.time()

    resultado_ademacor = importar_aporte_ademacor(archivo_ademacor, anio, 'ADEMACOR', batch_size)

    resultado_famecor = None
    if archivo_famecor:
        resultado_famecor = importar_aporte_ademacor(archivo_famecor, anio, 'FAMECOR', batch_size)
